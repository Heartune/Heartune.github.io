---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* B.S. in Software Engineering, Beijing University of Posts and Telecommunications, 2022-2026 (expected)
  * School of Computer Science (National Pilot Software Engineering School)
  * Member of Ye Peida Innovation and Entrepreneurship Education Experimental Class
  * Major Ranking: 8/151 (expected final ranking: 3/151 with competition and research bonuses)
  * CET-4: 629, CET-6: 619

Research Interests
======
* Embodied Intelligence: World Models, Vision-Language-Action Models (VLA), 6D Object Pose Estimation, Modular Robotics, Multi-Robot Coordination
* Natural Language Processing: LLM Fine-tuning and Evaluation, Benchmark Construction
* Interdisciplinary Frontiers of Embodied Intelligence and Natural Language Processing

Research Papers (In Progress)
======
* **GAPartNet++: A Deep Learning-based Framework for Part 6D Pose Estimation**
  * First Author, paper nearly completed, submission imminent
  * Target: IEEE ICIP 2026 (CCF-B conference)
  * Focus: Part-level 6D pose estimation for robotic perception

* **ROBO-Bench: Benchmarking Large Language Models on Robotics Fundamentals Before Embodiment**
  * First Author, paper outline completed, expected completion in early January 2026
  * Target: IEEE/RSJ IROS 2026 (top-tier robotics conference, March 2026)
  * Focus: First theoretical foundation benchmark for LLMs in robotics

* **CNLaw-Bench: Constructing a Chinese Legal LLM Benchmark for Fine-grained Statute-level Evaluation**
  * First Author, data collection completed, currently performing data cleaning
  * Target: AAAI 2026 (top-tier AI conference)
  * Focus: Benchmark dataset for Chinese statute-level legal evaluation

* **CyberSecLLM: A Multi-Agent Mixture-of-Experts Framework for Large-Scale Cybersecurity Situational Awareness**
  * Third Author, currently advancing evaluation and paper work in parallel
  * Target: Nature Communications, expected completion by end of December 2025
  * Focus: Multi-agent mixture-of-experts framework for heterogeneous cybersecurity data

* **Cardiovascular and Cerebrovascular Medical Large Language Model Research** (in collaboration with School of Public Health, Tsinghua University)
  * Responsible for data collection, data cleaning, and Agentic RAG code implementation
  * Target: Nature/Science submission by year-end
  * Details remain confidential

Patents
======
* **A Modular Robot Assembly and Repair System Based on Multi-Manipulator Coordination**
  * First Inventor, passed preliminary review in September 2025
  * Chinese National Invention Patent

* **A Self-Production System for Modular Robots on Extraterrestrial Planets**
  * Second Inventor, passed preliminary review in September 2025
  * Chinese National Invention Patent

Publications
======
* **Modular Integrated Optimization of SSVEP for UGV Control**
  * Co-First Author
  * Accepted by IEEE International Conference on Robotics and Biomimetics (ROBIO, h5-index=16)
  * October 1, 2025

  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>

Research Experience
======
* 2024-Present: Research Assistant
  * Key Laboratory of Space Robotics Technology, Ministry of Education, Beijing University of Posts and Telecommunications
  * Advisor: Associate Professor Zhang Shiyu
  * Focus: Modular Robotics

* 2024-Present: Research Assistant
  * School of Computer and Information, Hefei University of Technology
  * Advisor: Associate Professor Liu Liu
  * Focus: 6D Object Pose Estimation

Work Experience
======
* Summer 2024: Computer Vision Intern
  * Hefei Xingbei Aviation Survey Information Technology Co., Ltd.
  * Projects: Infrared imaging, transmission line object recognition

* 2023-Present: Project Leader
  * Shahe University Park Smart Guide System Development
  * Cooperation with Beijing Changping District Government
  * Serving over 300,000 people annually

* 2023: Algorithm Developer
  * Guizhou Changshun County Science and Technology Poverty Alleviation Program
  * Responsible for rice disease and pest identification algorithms

Awards and Honors
======
* National Second Prize, 26th China Robot and Artificial Intelligence Competition
* Over ten national and provincial competition awards

Skills
======
* Programming Languages
  * Python, C++, Java, JavaScript
* Machine Learning & Deep Learning
  * PyTorch, TensorFlow
  * Computer Vision: Object Detection, 6D Pose Estimation
  * Natural Language Processing: LLM Fine-tuning, Evaluation
* Robotics
  * ROS, Gazebo
  * Modular Robotics, Multi-Robot Coordination
* Data Science
  * Data Collection, Data Cleaning, Data Analysis

Talks
======
  <ul>{% for post in site.talks reversed %}
    {% include archive-single-talk-cv.html  %}
  {% endfor %}</ul>

Teaching
======
  <ul>{% for post in site.teaching reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>

Service and Leadership
======
* Deputy Director, Science and Technology Innovation Practice Department, Computer Science School Youth League Committee
* League Branch Secretary
* Class Monitor
* Core Member, BUPT Blueprint and other research-oriented organizations
